...
#define FW_IMAGE_NAME "synaptics_touch_fw.img"
...
static int fwu_parse_image_info(void)
{
	...
	if (fwu->bl_version == BL_V7) {
		if (!fwu->img.contains_flash_config || fwu->img.fl_config.data == NULL) {
			dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		memset(fwu->image_name, '\0', MAX_IMAGE_NAME_SIZE);
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	fwu->image_name = kzalloc(MAX_IMAGE_NAME_SIZE, GFP_KERNEL);
	if (!fwu->image_name) {
		dev_err(rmi4_data->pdev->dev.parent,
