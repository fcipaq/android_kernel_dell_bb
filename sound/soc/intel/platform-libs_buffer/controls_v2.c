			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	// fcipaq
	buf = (char *) kzalloc(SST_MAX_BIN_BYTES, GFP_KERNEL);

	memcpy(buf, sst->byte_stream, SST_MAX_BIN_BYTES);

	// 512 bytes value.bytes.data[512]
	memcpy(ucontrol->value.bytes.data, buf, 512);
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	// 512 bytes value.bytes.data[512]
	memcpy(buf, ucontrol->value.bytes.data, 512);

	memcpy(sst->byte_stream, buf, SST_MAX_BIN_BYTES);
